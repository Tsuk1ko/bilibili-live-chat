...
$(document).ready(() => {
	...
	function onGift({
		...
		let appednHtml = giftNum => $main.append(`<div class="danmaku-item">${faceHTML}<div class="content"><span class="message">感谢</span><span class="author-name${isOwner?' owner':''}">${name}</span><span class="message">赠送的${giftNum}个</span><span class="gift">${gift.name}</span></div></div>`);
		...
		if (giftComb) {
			...
			gifts[uid][gift.id].timeout = setTimeout(() => {
				...
				giftQueue.push(() => appednHtml(total));
			}, giftComb);
		} else giftQueue.push(() => appednHtml(num));
	}
